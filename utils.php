<?php
  ...
  function existsDNIUser( $dni, $dbConn ) {

    if($dni != null){
        $sql = $dbConn->prepare("SELECT nombre  as name, apellido1 as lastname_1, email, telefono FROM usuarios_anda where dni like :dni AND activo = '1' ");
        ...
        if( $sql->rowCount()>0 ){
          ...
          $data = 'No existe usuarios con este DNI';
        }
    }
    else
      $data = 'Tiene que introducir dni';

    return $data;
  }

  function getAllShop( $dbConn ){
    $sql = $dbConn->prepare("SELECT s.name, s.direction, s.info, l.nombre as 'city' FROM shop s INNER JOIN provincias p ON s.id_provincia  = p.id_provincia
          INNER JOIN localidades l ON s.id_localidad  = l.id_localidad 
          WHERE activo = '1'; ");
    $sql->execute();
    ...
    if( $sql->rowCount()>0 ){
      ...
      $data = 'No hay tiendas con promociones actualmente';
    }
    return $data;
  }

  function getShopByPage( $page, $dbConn ){
    $page = $page -1;
    $initPage = $page * 10;
    $sql = $dbConn->prepare("SELECT s.name, s.direction, s.info, l.nombre as 'city' FROM shop s INNER JOIN provincias p ON s.id_provincia  = p.id_provincia
      INNER JOIN localidades l ON s.id_localidad  = l.id_localidad 
      WHERE activo = '1' LIMIT ".$initPage.", 10 ");
    $sql->execute();
